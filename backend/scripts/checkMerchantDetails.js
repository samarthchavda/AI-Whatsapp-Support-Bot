...
const run = async () => {
  try {
    ...
    const orders = await Order.find({ admin: admin._id }).limit(5);
    console.log(`\n📦 Total Orders Found: ${await Order.countDocuments({ admin: admin._id })}`);
    orders.forEach((o, i) => {
      console.log(`  Order ${i + 1}: #${o.orderNumber} | Customer: ${o.customerName || 'N/A'} | Phone: ${o.customerPhone || 'N/A'} | Email: ${o.customerEmail || 'N/A'}`);
    });

    const carts = await AbandonedCart.find({ admin: admin._id }).limit(5);
    console.log(`\n🛒 Total Abandoned Carts Found: ${await AbandonedCart.countDocuments({ admin: admin._id })}`);
    carts.forEach((c, i) => {
